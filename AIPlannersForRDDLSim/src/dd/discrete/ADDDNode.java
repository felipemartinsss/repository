...
public class ADDDNode extends ADDNode implements Comparable {
	...
	@Override
	public int compareTo(Object nodeAux) {
		if (nodeAux instanceof ADDDNode) {
			if (_dLower == _dUpper) {
				ADDDNode anotherNode = (ADDDNode) nodeAux;
				if (anotherNode._dLower == anotherNode._dUpper) {
					if (_dLower > anotherNode._dLower) {
						return 1;
					} else if (_dLower < anotherNode._dLower) {
						return -1;
					} else {
						return 0;
					}
				} else {
					return 0; // need to order the ranges
				}
			} else {
				return 0; // need to order the ranges
			}
