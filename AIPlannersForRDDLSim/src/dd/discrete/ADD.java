...
public class ADD extends DD {
	...
	/**
	 * Recebe um valor booleano e cria um nodo bin�rio com o valor,
	 * devolvendo o id desse nodo.
	 * @param bval
	 * @param create
	 * @return
	 */
	...
	public int pruneNodes(int id) {
		...
		if ((PRUNE_TYPE != REPLACE_AVG && PRUNE_TYPE != REPLACE_MIN && 
				PRUNE_TYPE != REPLACE_MAX && PRUNE_TYPE != REPLACE_RANGE)) {
	...
	public HashMap compressLeavesUsingRanges (HashSet <ADDDNode> leaves) {
		ArrayList collect = new ArrayList();
		MapList remap = new MapList(false, true /*hash*/);
		HashMap ret_map = null; 
		
		// Collect all distinct leaves... if approximate match in 
		// 'collect' then make note in map, otherwise add it to 'collect'
		ArrayList <ADDDNode> listOfLeaves = new ArrayList <ADDDNode> ();
		listOfLeaves.addAll(leaves);
		Collections.sort(listOfLeaves);
		LinkedHashSet <ADDDNode> orderedLeaves = new LinkedHashSet <ADDDNode> ();
		orderedLeaves.addAll(listOfLeaves);
		
		
		Iterator i = orderedLeaves.iterator();
		collect.clear();
		...
		while (i.hasNext()) {
			...
			for (Iterator j = collect.iterator(); d1 != null && j.hasNext();) {
				...
				if (Math.abs(d1._dLower - d2._dLower) <= PRUNE_PRECISION) {
		...
		// Now build HashMap to remap
			...
			while (i.hasNext()) {
				...
				double replaceLowerVal = common._dLower;
				double replaceUpperVal = common._dUpper;
				double min = common._dLower;
				double max = common._dUpper;
				for (Iterator j = remapped.iterator(); j.hasNext();) {
					ADDDNode d = (ADDDNode) j.next();
					// replace_val += d._dLower;
					max = Math.max(max, d._dUpper);
					min = Math.min(min, d._dLower);
				}
				...
				ADDDNode new_dnode = (ADDDNode) this.getNode(this.getDNode(
						replaceLowerVal, replaceUpperVal, true));
	...
	public HashMap compressLeaves(HashSet <ADDDNode> leaves) {

		ArrayList collect = new ArrayList();
		MapList remap = new MapList(false, true /*hash*/);
		HashMap ret_map = null; 
		if (PRUNE_TYPE == REPLACE_RANGE) {
			ret_map = compressLeavesUsingRanges(leaves);
		} else {
    ...
    /* Exemplo decomo percorrer nodos */
	...
	public Pair<Double, Double> completeEvaluate(int id, ArrayList assign) {
		...
		while (cur instanceof ADDINode) {
			...
			if ((level < assign.size())
				...
				Pair <Double, Double> p = new Pair <Double, Double>(Double.NaN, Double.NaN);
				return p;
			}
		}
		...
		// If get here, cur will be an ADDDNode, ADDBNode
		if (cur instanceof ADDDNode) {
			Pair <Double, Double> p = new Pair <Double, Double>( ((ADDDNode) cur)._dLower,  ((ADDDNode) cur)._dUpper);
			return p;
		} else if (cur instanceof ADDBNode) {
			return ((ADDBNode) cur)._bVal ? new Pair <Double, Double>(1.0d, 1.0d) : new Pair <Double, Double>(0.0d, 0.0d);
		} else {
