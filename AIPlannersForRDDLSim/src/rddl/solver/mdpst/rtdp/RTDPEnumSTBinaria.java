...
public class RTDPEnumSTBinaria extends Policy {
	...
	// Constructors
	public RTDPEnumSTBinaria() { }
	
	public RTDPEnumSTBinaria(String instance_name) {
		super(instance_name);
	}
	...
	public static ArrayList<ProbSet> normalizaProbSet(ArrayList<ProbSet> alSetProb) {
		double soma = 0.0d;
		for (int i = 0; i < alSetProb.size(); i++) {
			soma += alSetProb.get(i).getProbability();
		}
		
		for (int i = 0; i < alSetProb.size(); i++) {
			alSetProb.get(i).setProbability (alSetProb.get(i).getProbability() / soma);
		}
	...
	public static ArrayList <ProbSet> contaminacaoEpsilon (ArrayList <ProbState> probStates, double epsilon) {
		...
		for (int i = 0; i < probStates.size(); i++) {
			...
			if (i < probStates.size() - 1) {
				currentProbSet.setProbability ((probStates.get(i)._dProb - probStates.get(i + 1)._dProb) * epsilonComplement);
			} else {
				currentProbSet.setProbability (0.0d);
			}
		...
		normalizaProbSet (probSets);
		
		for (int i = probStates.size() - 2; i >= 0; i--) {
			ProbSet currentProbSet = probSets.get(i);
			currentProbSet.setProbability (probStates.get(i)._dProb + probStates.get(i + 1)._dProb);
		}
			
		probSets.get(0).setProbability(1.0d);
		
		return probSets;
	}
	
	public void imprimeProbSets (ArrayList <ProbSet> probSets) {
		for (int i = 0; i < probSets.size(); i++) {
			System.out.println(probSets.get(i).getStatesIn() + " " + probSets.get(i).getProbability());
		}
	}
	...
	public Pair<Double, ArrayList<ProbSet>> getQValue (ArrayList<Boolean> currentState, CString action) {
		...
		for (int indice = 0; indice < probSets.size() - 1; indice++) {
			...
			if (stateIterator.hasNext()) {
				...
				double prob = reachableSet.getProbability() -  probSets.get(indice + 1).getProbability();
	...
	private ArrayList<ProbState> computeSuccesorsProbEnum(ArrayList<Boolean> state, HashMap<Integer, Integer> iD2ADD) {
		...
		for (int i = 0; i <= _context._alOrder.size(); i++)
			assign.add(null);
		alEstadoProb.add (new ProbState(0.0d, null));
		inOrderSearch(_context.getNode(multCPTs), assign, alEstadoProb);
		// _context.getGraph(multCPTs).launchViewer();
		alEstadoProb.get(0)._dProb = 1.0;
		// alEstadoProb = normaliza (alEstadoProb);
		return alEstadoProb;
	}
	
	
	public static ArrayList<ProbState> normaliza(ArrayList<ProbState> alEstadoProb) {
		double soma = 0.0d;
		...
		if (node != null) {
			if (node instanceof ADDINode) {
				...
				if (probabilidade > 0.0d) {
					Double probabilidadeAnterior = (alEstadoProb.isEmpty() ? 0.0d : alEstadoProb.get(0)._dProb);
					ArrayList <Boolean> newAssign = new ArrayList <Boolean>();
					newAssign.addAll(assign);
					ProbState novoNo = new ProbState (probabilidadeAnterior + probabilidade, newAssign);
	...
	public ArrayList <Boolean> SortearEstado(ArrayList<ProbSet> alSetProb) {
		int inicio = 0;
		int fim = alSetProb.size() - 1; // último conjunto aparece apenas para preencher probabilidade 0. Não é um conjunto alcançável.
		int meio = (inicio + fim) / 2;
		System.out.println("ArrayList Size = " + alSetProb.size());
		...
		ProbSet upper = alSetProb.get(meio);
		ProbSet lower = alSetProb.get(meio + 1);
		int indice = 0;
		
		while ((rand > upper.getProbability() || rand <= lower.getProbability()) && ((fim - inicio) > 1)) {
			// if (indice > 15)
			//	System.out.println(indice);
			if (rand > upper.getProbability()) {
				fim = meio;
			} else {
				inicio =  meio;
			}
			meio = (inicio + fim) / 2;
			upper = alSetProb.get(meio);
			lower = alSetProb.get(meio + 1);
			indice++;
			System.out.println("Indice = " + indice);
		}
		
		HashSet <ArrayList <Boolean>> conjuntoSorteado = upper.statesIn;
		System.out.println("Conjunto sorteado: " + conjuntoSorteado);
		
		if (conjuntoSorteado == null) {
			conjuntoSorteado = alSetProb.get(0).getStatesIn();
		}
		...
		while (iteradorEstados.hasNext()) {
			...
			if (contador == numeroEstadoSorteado) {
				if (estadoSorteado != null) {
					...
					// Tratamento para estado nulo.
					numeroEstadoSorteado++;
				}
			} else {
				contador++;
			}
		...
		if (estadoSorteado == null) {
			System.out.println("Sorteio incorreto.");
			System.exit(1);
		} else {
			System.out.println("Estado sorteado: " + estadoSorteado);
		}
