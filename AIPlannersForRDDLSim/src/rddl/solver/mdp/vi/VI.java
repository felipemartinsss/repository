...
public class VI extends Policy {
	...
	public static int SOLVER_TIME_LIMIT = 600; // Solver time limit (seconds)
	...
	public ArrayList<PVAR_INST_DEF> getActions(State s) throws EvalException {
		...
		if (_hmAct2Regr == null) {
			...
			if (getActionCounter == 0) {
				initialState = (ArrayList<Boolean>) add_state_assign.clone();
			}
			getActionCounter++;
	...
	private Cronometro timeSPUDD = null;
	...
	private int valueDDFirstIteration = 0;
	
	public void spuddInfo (Cronometro c, double reward) {
		StringBuffer spuddInfoContent = new StringBuffer("");
		GeradorDeArquivo geradorDeArquivo = new GeradorDeArquivo (spuddInfoContent);
		ProblemDescription problemDescription = new ProblemDescription (_context, _hmActionName2Action);
		spuddInfoContent.append("===============================================================\n");
		spuddInfoContent.append("===================== SPUDD Report =====================\n");
		spuddInfoContent.append("===============================================================\n");
		HashSet valueFunctionLeaves = new HashSet ();
		_context.collectLeaves(_valueDD, valueFunctionLeaves);
		spuddInfoContent.append("Instance " + _rddlInstance._sName + "\n");
		// spuddInfoContent.append("Number of state variables: " + problemDescription.getNumVars() + "\n");
		// spuddInfoContent.append("Number of states: " + problemDescription.getNumStates() + "\n");
		// spuddInfoContent.append("Number of actions: " + problemDescription.getNumActions() + "\n");
		// spuddInfoContent.append("Number of Value Function Leaves in 1st iteration: " + valueDDFirstIteration + "\n");
		// spuddInfoContent.append("Number of Value Function Leaves in 2nd iteration: " + valueDDSecondIteration + "\n");
		// spuddInfoContent.append("Number of Value Function Leaves: " + valueFunctionLeaves.size() + "\n");
		// BigDecimal bd = new BigDecimal (valueFunctionLeaves.size());
		// spuddInfoContent.append("Ratio of the leaves w.r.t. the original mdp size: " + bd.divide(problemDescription.getNumStates()) + "\n");
		spuddInfoContent.append("Time to compute SPUDD (offline) (s): " + ((double) c.getTotalMilisegundos() / 1000.0d) + "\n");
		spuddInfoContent.append("Total Reward = " + reward + "\n");
		spuddInfoContent.append("V(s0) = " + _context.evaluate(_valueDD, initialState) + "\n");
		spuddInfoContent.append("===============================================================\n");
		spuddInfoContent.append("===============================================================\n\n");
		geradorDeArquivo.geraArquivo("SPUDDInfoLogs\\" + _rddlInstance._sName + "_SPUDDInfo.txt");
	}
	
	public void roundEnd(double reward) {
		spuddInfo (timeSPUDD, reward);
		System.out.println("\n*********************************************************");
	...
	private int valueDDSecondIteration;
	...
	public int solve(int time_limit_secs) throws TimeOutException {
		...
		while (_nIter < _nHorizon) {
			...
			_context.setPrunePrecision(1.0d);
			...
			if (_nIter == 1) {
				HashSet valueFunctionLeaves = new HashSet ();
				_context.collectLeaves(_valueDD, valueFunctionLeaves);
				valueDDFirstIteration = valueFunctionLeaves.size();
			} else if (_nIter == 2) {
				HashSet valueFunctionLeaves = new HashSet ();
				_context.collectLeaves(_valueDD, valueFunctionLeaves);
				valueDDSecondIteration = valueFunctionLeaves.size();
			}
