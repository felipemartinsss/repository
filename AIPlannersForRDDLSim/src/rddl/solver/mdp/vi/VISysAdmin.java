...
public class VISysAdmin extends Policy {
	...
	public VISysAdmin(String instance_name) {
		super(instance_name);
	}
	...
	public void roundInit(double time_left, int horizon, int round_number, int total_rounds) {
		...
		if (_translation == null) {
			...
			for (String a : _translation._hmActionMap.keySet()) {
					...
					if (probabilidade != 0) { 
						...
						for (int j = 1; j < atribuicao.size(); j++) {
							if (atribuicao.get(j) == null) {
								...
								// listaMapeamentoProbabilidades.remove(atribuicao);
				...
				for (int i = 0; i < listaMapeamentoProbabilidades.size(); i++) {
					...
					if (probabilidade != 0) {
						...
						/*
						System.out.println("P(" + mapeamentoAtribuicaoParaEstado.get(atribuicaoEstadoDestino) + "|" 
											+ mapeamentoAtribuicaoParaEstado.get(atribuicaoEstadoOrigem) + ", " 
											+ a + ") = " + probabilidade);
											*/
			...
			/*	
			for (ArrayList <Boolean> atribVariaveis : mapeamentoAtribuicaoParaEstado.keySet()) {
				System.out.println(atribVariaveis + " => " + mapeamentoAtribuicaoParaEstado.get(atribVariaveis));
			}
			*/
