...
import rddl.solver.mdp.refactored.planners.LRTDPUtils;
...
/**
 * An LRTDP implementation that is used to solve a 
 * reduced model obtained from a stochastic bisimulation.
 * This reduced model has all MDP states given 
 * an initial state s0. However, LRTDP uses only 
 * the reachable ones.
 */
public class MRFSWithLRTDP extends Policy {
	...
	private ArrayList<Integer> _alSaveNodes; // Nodes to save during cache
												// Constructors
	private LRTDPUtils lrtdpUtils;
	
	
	public MRFSWithLRTDP() {
	}

	public MRFSWithLRTDP(String instance_name) {
		super(instance_name);
	}
	...
	public ArrayList<PVAR_INST_DEF> getActions(State s) throws EvalException {
		...
		if (getActionCounter == 0) {
			...
			mdp.setValueFunction(valueFunction);
			lrtdpUtils = new LRTDPUtils(_sInstanceName, mdp);
		}
		...
		if (getActionCounter == 0) {
			if (ONLY_BISIMULATION) {
				...
				try {
					lrtdpUtils.setSolved(solved);
					lrtdpUtils.setPi(pi);
					mdp.setValueFunction(valueFunction);
					mdp.setHeuristicaAdmissivel(heuristicaAdmissivel);
					if (PLANEJAMENTO == OFFLINE && getActionCounter == 0) {
						lrtdpUtils.doLRTDP(currentAbstractState, bellmanError, 40);
	...
	/**
	 * Show the total memory reserved to Simulator: free memory.
	 * @return
	 */
	...
	/**
	 * Set the time given to simulator.
	 */
