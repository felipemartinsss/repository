...
public class sBRTDP_MaxVar_Hplus_Texp_Prune_Sampling extends Policy {
    ...
    // Constructors
    public sBRTDP_MaxVar_Hplus_Texp_Prune_Sampling() { }
    
    public sBRTDP_MaxVar_Hplus_Texp_Prune_Sampling(String instance_name) {
        super(instance_name);
    }
    ...
    public void roundInit(double time_left, int horizon, int round_number, int total_rounds) {
        ...
        if (_nRound > 1)
        	prune(1d);
        _nProf = _nRemainingHorizon;//_nRound<=10?10:_nRound<=20?7:5;
    ...
    public void prune(double mergeerror){
		_VUpper = pruneNodesValue(_VUpper, mergeerror);
		_VLower = pruneNodesValue(_VLower, mergeerror);
		System.out.println("Poda");
		//_VGap = pruneNodesValue(_VGap, mergeerror);
	}
    ...
    public HashMap<Integer, Pair<Double, Double>> _hmVarWeight;//weights for lower and upper branch Vgap
    public HashMap<Integer, Pair<Double, Double>> _hmNodeWeight;
    ...
    public void resetSolver() {
        ...
        _hmVarWeight = new HashMap<Integer, Pair<Double, Double>>();
    ...
    private ArrayList samplingVGap(HashMap<Integer, Integer> iD2ADD, ArrayList state) {
        ...
        for (CString s : _alStateVars) {
            ...
            Boolean condition = (probTrue>0.2d && probTrue<0.8d ? ran<=probTrue : probTrue>=0.8d);
    ...
    public void doRTDP(double time_limit_secs, ArrayList init_state) {
        ...
        try {
            ...
            _nTrials = 0;
            while(true) {
                doRTDPTrial((_nRemainingHorizon/(_nTrials+1)), init_state);
                _nTrials++;
            }
    ...
    public void doRTDPTrial(int trial_depth, ArrayList init_state) throws TimeOutException {
        ...
        for (int steps_to_go = _nTrials + 1; steps_to_go > 0 && cur_state != null; steps_to_go--) {
            ...
            //_VGap = DDUtils.insertValueInDD3(_VGap, cur_state, resU._dBestQValue - resL._dBestQValue, it, _translation._hmPrimeRemap, _context, _hmActionName2Action.get(resU._csBestAction)._hmVarID2CPT, _hmNodeWeight , weight,_hmVarWeight);
            ...
            Action a =  _hmActionName2Action.get(resU._csBestAction);
            cur_state = samplingVGap(a._hmVarID2CPT, cur_state);
	...
	public HashMap compressLeaves(ArrayList hsLeaves, double mergeError) {
		...
		for (int i=0;i<hsLeaves.size();i++){
			...
			if (nodoprun._dValue <= max){				 
				...
				max = nodoprun._dValue + mergeError;
				soma = nodoprun._dValue;
				numElem = 1;
			}
