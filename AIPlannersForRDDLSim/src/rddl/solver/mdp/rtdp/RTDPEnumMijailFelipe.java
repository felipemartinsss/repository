...
public class RTDPEnumMijailFelipe extends Policy {
	
	public static int SOLVER_TIME_LIMIT_PER_TURN = 6; // Solver time limit (seconds)
	...
	private double heuristicaAdmissivel = 0.0d;
		
	// Constructors
	public RTDPEnumMijailFelipe() { }
	
	public RTDPEnumMijailFelipe(String instance_name) {
		super(instance_name);
	}
	...
	public static void imprimeProbState (ArrayList <ProbState> probStates) {
		for (int i = 0; i < probStates.size(); i++) {
			...
			if (probStates.get(i)._dProb > 1.0d) {
				System.out.println("Probabilidades extraidas incorretamente.");
	...
	public Pair<Double, ArrayList<ProbState>> getQValue(ArrayList cur_state, CString action) {
		...
		// imprimeEstadosAlcancaveis (ProbsStates);
		// imprimeProbState (ProbsStates);
		...
		for (int indice = 0; indice < ProbsStates.size() - 1; indice++) {
			...
			if (!_valueDD.containsKey(pS._alState)) {
				_valueDD.put(pS._alState, heuristicaAdmissivel);
			}
	...
	public ArrayList<Boolean> SortearEstado(ArrayList<ProbState> alEstadoProb) {
		int inicio = 0;
		int fin = alEstadoProb.size() - 2; // último estado aparece apenas para preencher probabilidade 0. Não é um estado alcançável.
