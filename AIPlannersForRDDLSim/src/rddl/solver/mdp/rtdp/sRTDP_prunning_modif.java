...
public class sRTDP_prunning_modif extends Policy {
	...
	public double _nmergeError = 0.035d;
	...
	// Constructors
	public sRTDP_prunning_modif() { }
	
	public sRTDP_prunning_modif(String instance_name) {
		super(instance_name);
	}
	...
	public ArrayList<PVAR_INST_DEF> getActions(State s) throws EvalException {
		if (_nGetActions>1)
			prune(_nmergeError);//_nmergeError);
	...
	public int _nTrials;
	public double _dMinReward; // Important if approximating
	...
	public void resetSolver() {
		...
		for (Action a : _hmActionName2Action.values())
			_dMinReward = Math.max(_dMinReward, 
		...
		double value_range = (_dDiscount < 1d)? 
			 _dMinReward / (1d - _dDiscount) // being lazy: assume infinite horizon
			: _nHorizon * _dMinReward;        // assume max reward over horizon
		_nmergeError = Math.abs(value_range*_ndelta);
		System.out.println("Max:"+value_range);
		_valueDD = _context.getConstantNode(_dMinReward);//value_range);
	...
	public void doRTDPTrial(int trial_depth, ArrayList init_state) throws TimeOutException {
	    //System.out.println(init_state.size());
		//CString best_action_init_state = null;
		...
		for (int steps_to_go = _nTrials+1; steps_to_go > 0; steps_to_go--) {
	...
	public HashMap compressLeaves(ArrayList hsLeaves, double mergeError) {
		...
		int numElem = 0;
		idnodo = ((PrunningNode)hsLeaves.get(0))._nid;
		for (int i=hsLeaves.size()-1;i>=0;i--){/*for (int i=0;i<hsLeaves.size();i++){*/
	...
	public void mostrarhojas(int dd){
		if (!Lista.contains(dd)){
			...
			if (node != null) {
				if (node instanceof ADDINode) {
					...
					mostrarhojas(lowNode._nLocalID);
					Lista.add(lowNode._nLocalID);
					mostrarhojas(highNode._nLocalID);
					Lista.add(highNode._nLocalID);
				} else if (node instanceof ADDDNode) {
					ADDDNode leaf = (ADDDNode) node;
					System.out.print(leaf._dLower+", ");
				}
			}
		}
	}
	
	public Boolean isRewardLock(ArrayList state){
		double racum = 0;
		Action a =_hmActionName2Action.get(_alActionNames.get(_rand.nextInt(_alActionNames.size())));
		return checkRewardLock(state, _context.evaluate(a._reward, state));
	}
	
	public Boolean checkRewardLock(ArrayList state, double r){
		for (Map.Entry<CString, Action> me : _hmActionName2Action.entrySet()) {
			Action a = me.getValue();
			if (_context.evaluate(a._reward, state) != r) return false;			
			if (!checkRewardLock(sampleNextState(state, a._csActionName),r)) return false;
		}
		return true;
	}
	///////////////////////////////////////////////////////////////////////
