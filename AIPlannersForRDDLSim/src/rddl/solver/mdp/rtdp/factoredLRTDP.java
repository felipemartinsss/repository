...
public class factoredLRTDP extends Policy {
	
	// public static double SOLVER_TIME_LIMIT_PER_TURN = 1.2d; // Solver time limit (seconds) DEFAULT
	public static double SOLVER_TIME_LIMIT_PER_TURN = 172800.0d;
	...
	// Constructors
	public factoredLRTDP() { }
	
	public factoredLRTDP(String instance_name) {
		super(instance_name);
	}
	...
	public ArrayList<PVAR_INST_DEF> getActions(State s) throws EvalException {
		...
		SOLVER_TIME_LIMIT_PER_TURN = 172800d;
		if (s == null) {
		...
		if (getActionCounter == 0) {
			...
			QUpdateResult res = getBestQValue(add_state_assign);
			if (_csBestActionInitState == null) {
	...
	public void factLRTDPInfo (Cronometro c, double reward) {
		...
		ProblemDescription problemDescription = new ProblemDescription (_context, _hmActionName2Action);
		bisimulationInfoContent.append("===============================================================\n");
		bisimulationInfoContent.append("===================== Fact-LRTDP Report =====================\n");
		bisimulationInfoContent.append("===============================================================\n");
		HashSet valueFunctionLeaves = new HashSet ();
		_context.collectLeaves(_valueDD, valueFunctionLeaves);
		bisimulationInfoContent.append("Instance " + _rddlInstance._sName + "\n");
		...
		bisimulationInfoContent.append("Number of Value Function Leaves: " + valueFunctionLeaves.size() + "\n");
		BigDecimal bd = new BigDecimal (valueFunctionLeaves.size());
		bisimulationInfoContent.append("Ratio of the leaves w.r.t. the original mdp size: " + bd.divide(problemDescription.getNumStates()) + "\n");
		bisimulationInfoContent.append("Time to compute fact-LRTDP (offline) (s): " + ((double) c.getTotalMilisegundos() / 1000.0d) + "\n");
		bisimulationInfoContent.append("Total Reward = " + reward + "\n");
		bisimulationInfoContent.append("===============================================================\n");
		bisimulationInfoContent.append("===============================================================\n\n");
		geradorDeArquivo.geraArquivo("factoredLRTDPInfoLogs\\" + _rddlInstance._sName + "_factLRTDPInfo.txt");
	}

	
	public void roundEnd(double reward) {
		factLRTDPInfo (timeFactLRTDP, reward);
		System.out.println("\n*********************************************************");
	...
	public void resetSolver() {
		...
		double value_range = (_dDiscount < 1d) 
			? _dRewardRange / (1 - _dDiscount) // / (1d - _dDiscount) // being lazy: assume infinite horizon ((1 - Math.pow(_dDiscount, 40)) 
			: _nHorizon * _dRewardRange;       // assume max reward over horizon
		_dValueRange = value_range;// _dRewardRange;
		_valueDD = _context.getConstantNode(value_range);//_dRewardRange*_nHorizon);//
	}
	
	private Cronometro timeFactLRTDP = null;

	// Main RTDP Algorithm
	public void doRTDP(double time_limit_secs, ArrayList init_state) {
		Cronometro tempoPlanejamento = new Cronometro();
		tempoPlanejamento.setStart();
		...
		try {
			...
			// _nTrials = 0; DEFAULT
			_nTrials = 0;
			if (_context.evaluate(_nSolvedADD, init_state) != 0.0d){ // first time through will update
				QUpdateResult res = getBestQValue(init_state);
				_csBestActionInitState = res._csBestAction;
			}
			
			timeFactLRTDP = new Cronometro();
			timeFactLRTDP.setStart();
			while(_context.evaluate(_nSolvedADD, init_state) == 0.0d) {//_context.evaluate(_nSolvedADD, init_state) == 0.0d) {
				Cronometro timeForTrial = new Cronometro();
				timeForTrial.setStart();
				doRTDPTrial(40, init_state);
				_nTrials++; 
				timeForTrial.setEnd();
				System.out.println("Trial: " + _nTrials + " - Time: " + timeForTrial.getIntervalo());
			}
			timeFactLRTDP.setEnd();
			System.out.println("Tempo total fact-LRTDP: " + timeFactLRTDP.getIntervalo());
			...
			tempoPlanejamento.setEnd();
			System.out.println("Tempo de Planejamento: " + tempoPlanejamento.getIntervalo());
	...
	public void doRTDPTrial(int trial_depth, ArrayList init_state) throws TimeOutException {
		...
		for (int steps_to_go = trial_depth; steps_to_go > 0 /*&& _context.evaluate(_nSolvedADD, cur_state) == 0.0d/* && !_alGoalStates.contains(cur_state)*/; steps_to_go--) {
		...
		/*	if (_nSolvedADD == _context.getConstantNode(1))
				break;
			cur_state = chooseNextState(cur_state, res._csBestAction);
			if (cur_state == null)
				break;*/
		}
    ...
    public ArrayList chooseNextState(ArrayList current_state, CString action) {
    	reduceRemapLeavesCache = new Hashtable();
    	//_context.getGraph(_nSolvedADD).launchViewer();
    	Integer multCPTs= NegativeAdd(_nSolvedADD);
    	//_context.getGraph(multCPTs).launchViewer();
    	HashMap<Integer, Integer> iD2ADD = _hmActionName2Action.get(action)._hmVarID2CPT;
		Integer xiprime, xi;
		...
		for (CString x : _alStateVars){		
			...
			current_state.set(levelprime, true);
			probTrue=_context.evaluate(cpt_a_xiprime,current_state);
			current_state.set(levelprime, null);			
			if (probTrue == 0d || probTrue == 1d){
				...
				//_context.getGraph(newCPT).launchViewer();
		...
		for (int i = 0; i <= num; i++)
			assign.add(i<vars? true: null);
		assign = ChooseStateAdd(_context.getNode(multCPTs), assign, 1d);
		return assign;
	}
