...
public class LRTDP extends Policy {
	...
	public LRTDP(String instance_name) {
		super(instance_name);
	}
	...
	public void doRTDPTrial(int trial_depth, ArrayList init_state) throws TimeOutException {
		...
		for (int steps_to_go = _nRemainingHorizon/*_nTrials + 1*/; steps_to_go > 0 && !_alGoalStates.contains(cur_state) && !_lSolvedStates.contains(cur_state); steps_to_go--) {
		...
		while(!visited_states.empty()) {
			...
			if (!LRTDPcheckSolved(cur_state, _nEpsilon, visited_states.size()-1)) 
				break;
		}
	...
	private boolean LRTDPcheckSolved(ArrayList cur_state, double epsilon, int depth)/*, QUpdateResult res )*/ throws TimeOutException {
		...
		while (!open.isEmpty()) {
			...
			ArrayList<ArrayList> lSucessors = computeSuccesorsProbEnum(cur_state, a._hmVarID2CPT);
			Iterator it = lSucessors.listIterator();
		    while(it.hasNext()){
		    	flushCaches(); // Only thing to keep is MDP def. and current vfun
				checkTimeLimit();
		    	ArrayList s_prime = (ArrayList)it.next();
		          if (!_lSolvedStates.contains(s_prime) && !aux.contains(s_prime)) {
		            open.push(s_prime);
		            aux.add(s_prime);
		          }
		        }
		...
		if (rv)
			...
			while (!closed.isEmpty()){
				...
				_valueDD = DDUtils.UpdateValue(_context, _valueDD, cur_state, resultQ._dBestQValue);
				_nContUpperUpdates++;
			}
		return rv;
	}
	...
	private ArrayList<ArrayList> computeSuccesorsProbEnum(ArrayList state,HashMap<Integer, Integer> iD2ADD) {
		...
		ArrayList<ArrayList> alEstadoProb = new ArrayList<ArrayList>();
