...
public class BRTDPOtrantoMartinsAlternativo extends Policy
{
	...
	// Dummy Constructors
	public BRTDPOtrantoMartinsAlternativo() { }

	public BRTDPOtrantoMartinsAlternativo(String instance_name) {
		super(instance_name);
	}
	...
	private void doBRTDP(ArrayList initState)
	{
		...
		catch (TimeOutException e) 
		{
			...
			// If we have 0 trials, give more time for the next trials
			if (trials == 0 ) 
			{
				if (SOLVER_TIME_LIMIT_PER_TURN <= 4) {
					SOLVER_TIME_LIMIT_PER_TURN = SOLVER_TIME_LIMIT_PER_TURN * 2;
					if (backwardsUpdateQ) {
						backwardsUpdateQ = false;
					}
				} else if (MAX_TRIAL_DEPTH >= 3) {
					MAX_TRIAL_DEPTH--;
				}
			}
		}
	...
	private void executeTrial(ArrayList initState, int trialDepth) throws TimeOutException 
	{
		...
		for (int i = trialDepth; i > 0; i--)
		{
			...
			if (i > 1)
				currState = chooseNextStateAlt(currState, upperResult.action, lowerValueOfDD, upperValueOfDD);
		}
		...
		context.reduce(upperValueOfDD);
		context.reduce(lowerValueOfDD);
	}
