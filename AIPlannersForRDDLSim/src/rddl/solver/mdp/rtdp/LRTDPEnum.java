...
import rddl.solver.mdp.rtdp.LRTDPEnum.QUpdateResult;
import rddl.translate.RDDL2Format;
import util.CString;
import util.Pair;

public class LRTDPEnum extends Policy {
	
	public static double SOLVER_TIME_LIMIT_PER_TURN = 8.0d; // Solver time limit (seconds)
	...
	public LRTDPEnum(String instance_name) {
		super(instance_name);
	}
	...
	public void doRTDPTrial(int trial_depth, ArrayList init_state) throws TimeOutException {
		...
		for (int depth = visited_states.size() - 1; depth >= 0; depth--) {
			...
			if (!LRTDPcheckSolved(cur_state, 0.1, visited_states.size()-1)) 
				break;
		}
	...
	public Pair<Double, ArrayList<ProbState>> getQValue(ArrayList cur_state, CString action) throws TimeOutException {
		...
		for (int indice =0;indice<ProbsStates.size()-1; indice++) {
			...
			if (!_valueDD.containsKey(pS._alState))
				_valueDD.put(pS._alState, _dRewardRange);
