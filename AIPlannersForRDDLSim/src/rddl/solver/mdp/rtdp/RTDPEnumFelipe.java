...
public class RTDPEnumFelipe extends Policy {
	...
	// Constructors
	public RTDPEnumFelipe() { }
	
	public RTDPEnumFelipe(String instance_name) {
		super(instance_name);
	}
	...
	public void resetSolver() {
		...
		/*
		double rewardUp = 0.0d;
		for (Action a : _hmActionName2Action.values()) {
			rewardUp = Math.max(rewardUp, _context.getMaxValue(a._reward));
		}
		
		System.out.println("M�xima recompensa: " + rewardUp);
		*/
		...
		/*
		if (_dDiscount <= 0.99d) {
			heuristicaAdmissivel = rewardUp / (1 - _dDiscount);
		} else {
			heuristicaAdmissivel = _nHorizon * rewardUp;
		}*/
		...
		// enumerar todos os estados poss�veis fornecendo a heur�stica
		// admiss�vel como valor.
	}
	...
	public Pair<Double, ArrayList<ProbState>> getQValue(ArrayList<Boolean> cur_state, CString action) {
		...
		if (!_valueDD.containsKey(cur_state)) {
			_valueDD.put(cur_state, heuristicaAdmissivel);
		}
		
		for (int indice = 0; indice < ProbsStates.size(); indice++) {
			ProbState pS = ProbsStates.get(indice);
			if (!_valueDD.containsKey(pS._alState)) {
				double heuristicaEstadoSeguinte = Double.MIN_VALUE;
				for (Action acao : _hmActionName2Action.values()) {
					if (heuristicaEstadoSeguinte < _context.evaluate(acao._reward, pS._alState)) {
						heuristicaEstadoSeguinte = _context.evaluate(acao._reward, pS._alState);
					}
				}
				heuristicaEstadoSeguinte = (_dDiscount < 1d) 
						? heuristicaEstadoSeguinte / (1d - _dDiscount) // being lazy: assume infinite horizon
						: _nHorizon * heuristicaEstadoSeguinte;        // assume max reward over horizon*/
				_valueDD.put(pS._alState, heuristicaEstadoSeguinte);
				System.out.println("Heuristica estado seguinte: " + heuristicaEstadoSeguinte);
				// _valueDD.put(pS._alState, _context.evaluate(a._reward, cur_state));
			}
			
			double valorEstadoSeguinte = _valueDD.get(pS._alState);
			double prob = pS._dProb;
			// System.out.print("(Pr = " + prob + ") * (" + valorEstadoSeguinte + ") ");
			soma += (prob * valorEstadoSeguinte);
		}
	 ...
	 * os estados que podem ser alcançados pela ação representada por esse ADD
	 * @param idADD
	 */
	/*public ArrayList<ProbState> extraiProbabilidades (Integer idADD, HashMap<ArrayList<Boolean>, Integer> keys){
		...
		// Número de mapeamentos (estado, probabilidade).
	 ...
	 * a probabilidade de ser alcançado.
	 */
	...
	public ArrayList <Boolean> SortearEstado(ArrayList<ProbState> alEstadoProb) {
		...
		int i;
		for (i = 0; i < alEstadoProb.size(); i++) {
			if ((inicio <= rand) && (rand < inicio + alEstadoProb.get(i)._dProb)) {
				estadoSorteado = alEstadoProb.get(i)._alState;
				break;
			} else {
				inicio += alEstadoProb.get(i)._dProb;
			}
		}
		
		if (estadoSorteado == null) {
			estadoSorteado = alEstadoProb.get(i - 1)._alState;
		}
