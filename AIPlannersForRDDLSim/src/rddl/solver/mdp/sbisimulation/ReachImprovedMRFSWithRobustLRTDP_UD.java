...
public class ReachImprovedMRFSWithRobustLRTDP_UD extends Policy {
	private static boolean ONLY_REACHABILITY_ANALYSIS = false;
	private static boolean ONLY_BISIMULATION = false;
	private boolean PARTIAL_REACHABILITY_ANALYSIS = false;
	...
	private static boolean ONLY_USEFUL_PARTITIONS = true; // N�o pode ser true se MODEL_MINIMIZATION = true.
	...
	// iguais => epsilon \in [0,1]f
	private boolean EPSILON_IN_SPLIT = true;
	private double EPSILON_MODEL_REDUCTION = 0.1d; // use 0.05 for the example_approx_givan
	...
	// Constructors
	public ReachImprovedMRFSWithRobustLRTDP_UD() {
	}
	...
	private int findReachableStates(ArrayList<Boolean> add_state_assign_clone, int horizon) {
		...
		for (int i = 0; i < 40; i++) {
			...
			// _context.getGraph(reachableStates).launchViewer();
 			...
 			flushCaches();
 			// if (PARTIAL_REACHABILITY_ANALYSIS  && ((double) RUNTIME.freeMemory() / (double) RUNTIME
			//		.totalMemory()) < 0.25) {
			//	break;
			// }
	...
	private void robustLRTDP(ArrayList <Boolean> currentAbstractState, double bellmanError, int horizon) throws TimeOutException {
		...
		System.out.println("Performing an lrtdp trial: number " + trials);
		...
		// System.exit(0);
		System.out.println("Performing an checksolved: number " + trials);
		while (!estadosVisitados.isEmpty()) {
			// System.out.println("Visited states: " + estadosVisitados.size());
			// checkTimeLimit();
	...
	private ArrayList<ProbState> getWorstModel(
		...
		*/
		...
		int validTransitionSize = transitions.size() - 1;
		while (i < validTransitionSize && sum < 0.999999d) {
		...
		if (ONLY_USEFUL_PARTITIONS) {
			if (EPSILON_MODEL_REDUCTION == 0.0d) {
				...
				if (EPSILON_IN_SPLIT && EPSILON_MODEL_REDUCTION > 0.0d) {
					String directory = "Reachability" + EPSILON_MODEL_REDUCTION + "AdequateMRPlannerTime";
	...
	private ArrayList<IntervalProbState> computeBMDPSuccessors(ArrayList <Boolean> state, ArrayList<ArrayList<Boolean>> completeStates,
		...
		if (minTransitionFunction.equals(maxTransitionFunction)) {
			for (int i = 0; i < transicoes.size(); i++) {
				transicoes.get(i)._dUpperProb = transicoes.get(i)._dLowerProb;
			}
		} else {
			minExtraction = false;
	...
	*/
	...
	public ArrayList <Integer> identifyAllRedundantPartitions (ArrayList <Integer> partitions) {
		...
		for (int i = 0; i < simplerPartitions.size(); i++) {
			...
			Integer tempPartition = simplerPartitions.get(i);
			if (EPSILON_MODEL_REDUCTION > 0) {
				_context.setPrunePrecision(EPSILON_MODEL_REDUCTION);
				_context.setPruneType(ADD.REPLACE_RANGE);
				tempPartition = _context.pruneNodes(tempPartition);
			}
			simplerPartitions.set(i , simplifyPartition (tempPartition, oldValueToCounting));
			partitionWORepetitionToSimplerPartition.put(partitionsWithoutRepetition.get(i), simplerPartitions.get(i));
			// _context.getGraph (partitionsWithoutRepetition.get(i)).launchViewer();
			// _context.getGraph (simplerPartitions.get(i)).launchViewer();
			// _context.getGraph (simplerPartitions.get(i)).launchViewer();
		}
	...
	public void roundEnd(double reward) {
		...
		System.out
				.println("*********************************************************");
		System.out.println("Discovered states: " + statesDiscovered);
	}
