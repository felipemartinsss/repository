...
public class FTVIOverStochasticBisimulation extends Policy {
	...
	// Constructors
	public FTVIOverStochasticBisimulation() {
	}
	...
	public Double backup (ArrayList <Boolean> state) {
		Double maxLowerQValue = -Double.MAX_VALUE;
		Double maxUpperQValue = -Double.MAX_VALUE;
		for (Action a : _hmActionName2Action.values()) {
			ArrayList<Boolean> atribb = (ArrayList<Boolean>) state.clone();
			ArrayList <ProbState> transicoes = computeSuccesorsEnum(atribb, a._hmVarID2CPT);
			...
			for (int i = 0; i < transicoes.size(); i++) {
				...
				currentActionUpperValue += probState._dProb * _context.evaluate(valueFunctionUpperBounds, probState.nextRepresentant);
				currentActionLowerValue += probState._dProb * _context.evaluate(valueFunctionLowerBounds, probState.nextRepresentant);
			}
			currentActionUpperValue = _context.evaluate(a._reward, state) + _dDiscount * currentActionUpperValue;
			currentActionLowerValue = _context.evaluate(a._reward, state) + _dDiscount * currentActionLowerValue;
			
			if (currentActionUpperValue < _context.evaluate(valueFunctionLowerBounds, state)) {
			...
			if (currentActionUpperValue > maxUpperQValue) {
				maxUpperQValue = currentActionUpperValue;
			}
			
			if (currentActionLowerValue > maxLowerQValue) {
				maxLowerQValue = currentActionLowerValue;
			}
		}
		Double oldValue = _context.evaluate(valueFunctionUpperBounds, state);
		valueFunctionUpperBounds = DDUtils.UpdateValue(_context, valueFunctionUpperBounds, state, maxUpperQValue);
		valueFunctionLowerBounds = DDUtils.UpdateValue(_context, valueFunctionLowerBounds, state, maxLowerQValue);
		
		return Math.abs(maxUpperQValue - oldValue);
	}
